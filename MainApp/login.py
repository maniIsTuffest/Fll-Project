...
st.sidebar.header("🏺 ArtiQuest.Ai")
...
if st.session_state["authentication_status"]:
    ...
    page_title = (
        f"{st.session_state['page']} | Admin Panel"
        if get_user_role(st.session_state["username"]) == "admin"
        else f"{st.session_state['page']} | User Dashboard"
    )
    ...
    if role == "admin":
        ...
        st.sidebar.title("Lab Engineer User Menu")
        if st.sidebar.button("Dashboard"):
            st.session_state["page"] = "User Dashboard"
        if st.sidebar.button("Gallery Artifacts"):
            st.session_state["page"] = "Gallery Artifacts"
    ...
    if st.session_state["page"] == "Admin Dashboard":
        ...
        # Role distribution cha
        role_counts = {}
        for u in users:
            role_counts[u[2]] = role_counts.get(u[2], 0) + 1
        st.subheader("User Role Distribution")
        df = pd.DataFrame.from_dict(role_counts, orient="index", columns=["Count"])
        st.bar_chart(df)
    ...
    # -------------------- Admin: User Management --------------------
        ...
        filtered_users = [
            u
            for u in users
            if search_query.lower() in u[0].lower()
            or search_query.lower() in u[3].lower()
        ]
    ...
    # -------------------- User Dashboard --------------------
        ...
        logs = [
            log for log in get_audit_logs() if log[1] == st.session_state["username"]
        ]
